...
class TestUserLegacyIdLookup:
    def test_requires_authentication(self):
        response = APIClient().get("/api/v1/reports/user-lookup/?legacy_ids=cuid-1")
        assert response.status_code == 401

    def test_forbidden_for_role_without_access(self, sin_acceso):
        response = _client_for(sin_acceso).get("/api/v1/reports/user-lookup/?legacy_ids=cuid-1")
        assert response.status_code == 403

    def test_400_without_legacy_ids(self, jefe):
        response = _client_for(jefe).get("/api/v1/reports/user-lookup/")
        assert response.status_code == 400

    def test_resolves_matching_users_and_ignores_unmatched_ids(self, jefe, coordinador):
        jefe.legacy_postgres_id = "cuid-jefe"
        jefe.save(update_fields=["legacy_postgres_id"])
        coordinador.legacy_postgres_id = "cuid-coord"
        coordinador.save(update_fields=["legacy_postgres_id"])

        response = _client_for(jefe).get(
            "/api/v1/reports/user-lookup/?legacy_ids=cuid-jefe,cuid-coord,cuid-does-not-exist"
        )
        assert response.status_code == 200
        by_legacy_id = {row["legacy_postgres_id"]: row["id"] for row in response.data}
        assert by_legacy_id == {"cuid-jefe": jefe.id, "cuid-coord": coordinador.id}

    def test_allows_coordinador_nacional_without_admin_permission(self, coordinador):
        """El permiso es `CanAccessReports`, no `usuarios.ver` — un
        Coordinador Nacional sin acceso a administración de usuarios
        igual puede generar reportes, así que este lookup no debe
        exigir ese permiso distinto."""
        coordinador.legacy_postgres_id = "cuid-coord"
        coordinador.save(update_fields=["legacy_postgres_id"])
        response = _client_for(coordinador).get("/api/v1/reports/user-lookup/?legacy_ids=cuid-coord")
        assert response.status_code == 200
