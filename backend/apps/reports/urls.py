...
solo persisten el resultado ya calculado. `user-lookup/` agregado en la
Fase 57 (ver docs/AUDIT_LOG.md § 2026-08-25) — puente cuid→id numérico
para el roster, primer paso de portar el CÁLCULO del snapshot."""
...
from .views import (
    ...
    UserLegacyIdLookupView,
)

urlpatterns = [
    # `list/`/`closure-status/`/`audit/`/`integrity-incidents/`/`*-team-kpis/`/
    # `user-lookup/` ANTES del catch-all `<str:report_id>/` — de otro modo el
    # converter `str` los capturaría como un `report_id` literal.
    ...
    path("user-lookup/", UserLegacyIdLookupView.as_view(), name="reports-user-lookup"),
