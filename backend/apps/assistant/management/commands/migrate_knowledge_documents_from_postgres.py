"""Importa `KnowledgeDocument` desde PostgreSQL (Next.js legacy) — Wave 1
de la migración de datos reales (ver docs/AUDIT_LOG.md § 2026-08-27, Fase
80). Depende solo de `User`. `DocumentChunk` (Wave 2) depende de este
comando por `documentId`.

El `KnowledgeDocument` de Prisma no tiene `updatedAt` (solo
`createdAt`), pero el de Django hereda `BaseModel` — sin un `updatedAt`
legacy real, se usa el mismo valor que `created_at`."""

from datetime import timezone as dt_timezone

from django.core.management.base import BaseCommand, CommandError
from django.utils import timezone

from apps.assistant.models import KnowledgeDocument
from apps.core.legacy_migration import (
    bulk_import_rows,
    fetch_legacy_rows,
    filter_not_yet_imported,
    legacy_postgres_connection,
    resolve_legacy_ids,
)
from apps.users.models import User


def _aware(value):
    if value is None:
        return None
    if timezone.is_naive(value):
        return timezone.make_aware(value, dt_timezone.utc)
    return value


class Command(BaseCommand):
    help = "Importa la Base de Conocimiento desde KnowledgeDocument de PostgreSQL (Next.js legacy)."

    def add_arguments(self, parser):
        parser.add_argument("--dry-run", action="store_true")

    def handle(self, *args, **options):
        connection = legacy_postgres_connection()
        try:
            rows = fetch_legacy_rows(
                connection,
                'SELECT id, title, "fileName", content, "githubPath", "githubSha", status, '
                '"processingError", "uploadedById", "createdAt" '
                'FROM "KnowledgeDocument" ORDER BY "createdAt"',
            )
        finally:
            connection.close()

        legacy_ids = [row["id"] for row in rows]
        pending_ids = filter_not_yet_imported(KnowledgeDocument, legacy_ids)
        pending_rows = [row for row in rows if row["id"] in pending_ids]
        skipped = len(rows) - len(pending_rows)

        if options["dry_run"]:
            self.stdout.write(self.style.SUCCESS(f"[dry-run] A importar: {len(pending_rows)}. Ya existentes: {skipped}."))
            return

        user_ids = resolve_legacy_ids(User, [row["uploadedById"] for row in pending_rows])

        instances = []
        failed = []
        for row in pending_rows:
            uploaded_by_id = user_ids.get(row["uploadedById"])
            if uploaded_by_id is None:
                failed.append(f"{row['id']}: usuario uploadedById no resuelto en Django")
                continue
            created_at = _aware(row["createdAt"])
            instances.append(
                KnowledgeDocument(
                    legacy_postgres_id=row["id"],
                    title=row["title"],
                    file_name=row["fileName"],
                    content=row["content"],
                    github_path=row["githubPath"],
                    github_sha=row["githubSha"],
                    status=row["status"],
                    processing_error=row["processingError"],
                    uploaded_by_id=uploaded_by_id,
                    created_at=created_at,
                    updated_at=created_at,
                )
            )

        created = bulk_import_rows(KnowledgeDocument, instances, batch_size=100)

        self.stdout.write(self.style.SUCCESS(f"Importados: {created}. Ya existentes (omitidos): {skipped}."))
        for message in failed:
            self.stdout.write(self.style.ERROR(f"  - {message}"))
        if failed:
            raise CommandError(f"{len(failed)} filas no se pudieron importar — ver detalle arriba.")
