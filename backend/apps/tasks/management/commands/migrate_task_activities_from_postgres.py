"""Importa `TaskActivity` desde PostgreSQL (Next.js legacy) — Wave 2 de
la migración de datos reales (ver docs/AUDIT_LOG.md § 2026-08-27, Fase
80). Depende de `Task` (Wave 1) + `User`. `ActivityComment`/
`ActivityAuditLog` (Wave 3) dependen de este comando por `activityId`.

Tabla potencialmente grande (un registro por hora trabajada, histórico
completo) — `batch_size` explícito más chico que el default."""

from datetime import timezone as dt_timezone

from django.core.management.base import BaseCommand, CommandError
from django.utils import timezone

from apps.core.legacy_migration import (
    bulk_import_rows,
    fetch_legacy_rows,
    filter_not_yet_imported,
    legacy_postgres_connection,
    resolve_legacy_ids,
)
from apps.tasks.models import Task, TaskActivity
from apps.users.models import User


def _aware(value):
    if value is None:
        return None
    if timezone.is_naive(value):
        return timezone.make_aware(value, dt_timezone.utc)
    return value


class Command(BaseCommand):
    help = "Importa registros de horas desde TaskActivity de PostgreSQL (Next.js legacy)."

    def add_arguments(self, parser):
        parser.add_argument("--dry-run", action="store_true")

    def handle(self, *args, **options):
        connection = legacy_postgres_connection()
        try:
            rows = fetch_legacy_rows(
                connection,
                'SELECT id, "taskId", "authorId", reason, "startTime", "endTime", duration, '
                'description, "isRetroactive", "activityDate", "adminComment", '
                '"modifiedByAdmin", "modifiedAt", "createdAt" '
                'FROM "TaskActivity" ORDER BY "createdAt"',
            )
        finally:
            connection.close()

        legacy_ids = [row["id"] for row in rows]
        pending_ids = filter_not_yet_imported(TaskActivity, legacy_ids)
        pending_rows = [row for row in rows if row["id"] in pending_ids]
        skipped = len(rows) - len(pending_rows)

        if options["dry_run"]:
            self.stdout.write(self.style.SUCCESS(f"[dry-run] A importar: {len(pending_rows)}. Ya existentes: {skipped}."))
            return

        task_ids = resolve_legacy_ids(Task, [row["taskId"] for row in pending_rows])
        user_ids = resolve_legacy_ids(User, [row["authorId"] for row in pending_rows])

        instances = []
        failed = []
        for row in pending_rows:
            task_id = task_ids.get(row["taskId"])
            author_id = user_ids.get(row["authorId"])
            if task_id is None or author_id is None:
                failed.append(f"{row['id']}: taskId/authorId no resuelto en Django")
                continue
            created_at = _aware(row["createdAt"])
            instances.append(
                TaskActivity(
                    legacy_postgres_id=row["id"],
                    task_id=task_id,
                    author_id=author_id,
                    reason=row["reason"],
                    start_time=row["startTime"],
                    end_time=row["endTime"],
                    duration=row["duration"],
                    description=row["description"] or "",
                    is_retroactive=row["isRetroactive"],
                    activity_date=_aware(row["activityDate"]),
                    admin_comment=row["adminComment"],
                    modified_by_admin=row["modifiedByAdmin"],
                    modified_at=_aware(row["modifiedAt"]),
                    created_at=created_at,
                    updated_at=created_at,
                )
            )

        created = bulk_import_rows(TaskActivity, instances, batch_size=100)

        self.stdout.write(self.style.SUCCESS(f"Importados: {created}. Ya existentes (omitidos): {skipped}."))
        for message in failed:
            self.stdout.write(self.style.ERROR(f"  - {message}"))
        if failed:
            raise CommandError(f"{len(failed)} filas no se pudieron importar — ver detalle arriba.")
