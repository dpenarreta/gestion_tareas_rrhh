"""Importa `IdeaVote` desde PostgreSQL (Next.js legacy) — Wave 2 de la
migración de datos reales (ver docs/AUDIT_LOG.md § 2026-08-27, Fase 80).
Depende de `ImprovementIdea` (Wave 1) + `User`.

El `IdeaVote` de Prisma no tiene `updatedAt` (solo `createdAt`), pero el
...
class Command(BaseCommand):
    help = "Importa votos de ideas desde IdeaVote de PostgreSQL (Next.js legacy)."
    ...
    def handle(self, *args, **options):
        ...
        try:
            rows = fetch_legacy_rows(
                connection,
                'SELECT id, "ideaId", "userId", "createdAt" FROM "IdeaVote" ORDER BY "createdAt"',
            )
        finally:
            connection.close()

        legacy_ids = [row["id"] for row in rows]
        pending_ids = filter_not_yet_imported(IdeaVote, legacy_ids)
        ...
        for row in pending_rows:
            ...
            if idea_id is None or user_id is None:
                failed.append(f"{row['id']}: ideaId/userId no resuelto en Django")
                continue
        ...
        created = bulk_import_rows(IdeaVote, instances, batch_size=200)
