"""Importa `LeaveRecord` desde PostgreSQL (Next.js legacy) — Wave 1 de la
migración de datos reales (ver docs/AUDIT_LOG.md § 2026-08-27, Fase 80).
Depende solo de `User`.

El `LeaveRecord` de Prisma no tiene `updatedAt` (solo `createdAt`), pero
el de Django hereda `BaseModel` — sin un `updatedAt` legacy real, se usa
el mismo valor que `created_at`."""
...
class Command(BaseCommand):
    help = "Importa Permisos/Ausencias desde LeaveRecord de PostgreSQL (Next.js legacy)."
    ...
    def handle(self, *args, **options):
        ...
        try:
            rows = fetch_legacy_rows(
                connection,
                'SELECT id, "userId", type, date, "isFullDay", "durationMinutes", observation, '
                '"createdBy", "createdAt" FROM "LeaveRecord" ORDER BY "createdAt"',
            )
        finally:
            connection.close()

        legacy_ids = [row["id"] for row in rows]
        pending_ids = filter_not_yet_imported(LeaveRecord, legacy_ids)
        ...
        created = bulk_import_rows(LeaveRecord, instances)
