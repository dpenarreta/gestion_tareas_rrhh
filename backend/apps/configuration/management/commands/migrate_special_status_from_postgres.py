"""Importa `SpecialStatus` desde PostgreSQL (Next.js legacy) — Wave 1 de
la migración de datos reales (ver docs/AUDIT_LOG.md § 2026-08-27, Fase
80). Depende solo de `User`.

El `SpecialStatus` de Prisma no tiene `updatedAt` (solo `createdAt`),
...
class Command(BaseCommand):
    help = "Importa Estados Especiales desde SpecialStatus de PostgreSQL (Next.js legacy)."
    ...
    def handle(self, *args, **options):
        ...
        try:
            rows = fetch_legacy_rows(
                connection,
                'SELECT id, "userId", type, "startDate", "endDate", "isActive", "dailyHours", '
                '"limitLow", "limitBase", "limitHigh", "limitOverload", "createdBy", "createdAt" '
                'FROM "SpecialStatus" ORDER BY "createdAt"',
            )
        finally:
            connection.close()

        legacy_ids = [row["id"] for row in rows]
        pending_ids = filter_not_yet_imported(SpecialStatus, legacy_ids)
        ...
        for row in pending_rows:
            ...
            user_legacy_ids.add(row["createdBy"])
        ...
        for row in pending_rows:
            ...
            created_by_id = user_ids.get(row["createdBy"])
            if user_id is None or created_by_id is None:
                failed.append(f"{row['id']}: usuario userId/createdBy no resuelto en Django")
                continue
            ...
            instances.append(
                SpecialStatus(
                    ...
                    limit_low=row["limitLow"],
                    limit_base=row["limitBase"],
                    limit_high=row["limitHigh"],
                    limit_overload=row["limitOverload"],
        ...
        created = bulk_import_rows(SpecialStatus, instances)
