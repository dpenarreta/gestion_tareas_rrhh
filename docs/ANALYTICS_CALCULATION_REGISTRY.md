...
| Recomendaciones de redistribución (equipo) | `computeTeamRecommendations` | `computeTeamCapacityForecast`, `capacityToScore`, `classifyCapacity`, `getEffectiveAnalyticsConfig`, `getAllEffectiveRoleCompatibility` (systemConfig.ts), `ROLE_LEVEL` (roles.ts, 2026-07-26 — Compatibilidad Organizacional) | `capacityForecast.ts` por miembro; `SystemConfigHistory` (Matriz de Compatibilidad Operativa) |
