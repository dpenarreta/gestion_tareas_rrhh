...
| **Consistencia** | ¿El mismo período, regenerado, produce los mismos números? ¿Pantalla y PDF coinciden siempre? | Cumplido estructuralmente — un solo Builder canónico, un solo render HTML compartido entre pantalla y PDF (`docs/AUDIT_LOG.md` § Decisión 10a). La narrativa de NOVA sí puede variar entre llamadas a Gemini (no determinista) — los NÚMEROS nunca varían, el TEXTO puede reformularse. |
