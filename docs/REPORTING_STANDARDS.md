...
| **Consistencia** | El mismo período, generado dos veces con los mismos filtros, produce los mismos números (no necesariamente el mismo Report ID ni la misma narrativa NOVA, que puede variar entre llamadas a Gemini). | Cero aleatoriedad en el cálculo determinista; solo NOVA tiene una superficie no determinista, y está delimitada a 4 campos narrativos, nunca a un número. |
