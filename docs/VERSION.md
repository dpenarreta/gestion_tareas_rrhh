...
|---|---|---|
| **NEXO** (producto) | **v1.20.1** | Ver `docs/CHANGELOG.md` para el detalle de qué introdujo cada versión |
| **Analytics Engine** | v1.5.0 | `ANALYTICS_ENGINE_VERSION` en `src/lib/analytics.ts` — sin cambios (el fix de Carga Laboral toca `/api/kpis/[userId]` y `/api/kpis/me`, no `src/lib/analytics.ts`) |
...
**Última actualización:** 2026-07-28 (v1.20.1 — Fix: el indicador "Carga Laboral" de Analytics/KPIs ahora usa la misma fuente que WorkloadCard/Equilibrio Operativo (`cargaTiempo.mensual`, Base Horaria Efectiva) en vez de un ratio horas-estimadas-vs-reales de tareas no relacionado)
**Autor:** Claude Code
