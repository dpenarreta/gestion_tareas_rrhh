...
|---|---|---|
| **NEXO** (producto) | **v1.20.0** | Ver `docs/CHANGELOG.md` para el detalle de qué introdujo cada versión |
| **Analytics Engine** | v1.5.0 | `ANALYTICS_ENGINE_VERSION` en `src/lib/analytics.ts` — sin cambios (la corrección de Compatibilidad Organizacional toca `computeTeamRecommendations`, que no tiene entrada en `FORMULA_VERSIONS` — no es una fórmula de score/KPI) |
...
**Última actualización:** 2026-07-26 (v1.20.0 — Motor Determinista de Recomendaciones: Compatibilidad Organizacional, Matriz de Compatibilidad Operativa configurable, `computeTeamRecommendations` nunca sugiere redistribución entre niveles jerárquicos distintos)
**Autor:** Claude Code
