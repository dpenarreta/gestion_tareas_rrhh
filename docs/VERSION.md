...
|---|---|---|
| **NEXO** (producto) | **v1.19.0** | Ver `docs/CHANGELOG.md` para el detalle de qué introdujo cada versión |
| **Analytics Engine** | v1.5.0 | `ANALYTICS_ENGINE_VERSION` en `src/lib/analytics.ts` — sin cambios (Sprint E construye una capa predictiva nueva y aislada, `trendEngine.ts`/`predictionEngine.ts`/`preventiveIntelligence.ts`, con sus propias versiones de módulo `"1.0.0"` — el Analytics Engine central no se toca) |
...
**Última actualización:** 2026-07-26 (v1.19.0 — Sprint E: Analytics Predictivo e Inteligencia Preventiva, Trend Engine + 4 predicciones explicables + Estabilidad Operativa + alertas preventivas + simulador de escenarios + Tendencias Históricas, módulo nuevo y autónomo `/inteligencia-preventiva`, cero cambios al Analytics Engine central)
**Autor:** Claude Code
