...
Nexo utiliza cuatro proveedores externos vigentes que procesan datos personales: **Google** (IA, API de Gemini), **GitHub** (almacenamiento documental), **Zoom** (videoconferencia) y, desde el 2026-09-11, **Alphaside** (hosting del dominio `laarcourierdocs.com`, servidor SMTP por el que salen los correos de recuperación de contraseña: recibe nombre, dirección de correo del titular y el enlace de un solo uso). Este último se incorporó al configurar el correo saliente: se había asumido que saldría por el servidor Zimbra propio de la empresa —lo que no habría constituido transferencia a un tercero— y resultó que el buzón que usa Nexo pertenece a un dominio alojado afuera. Queda por confirmar con ese proveedor la ubicación de sus servidores (a efectos de transferencia internacional, sección 3) y si existe un acuerdo de tratamiento vigente. Ninguno tiene hoy un acuerdo de encargado de tratamiento (o equivalente) formalizado. Esto es responsabilidad del área legal — el sistema no puede formalizar estos acuerdos por sí mismo (`docs/RAT.md`, sección 6). **Groq** (IA, proveedor anterior) se retiró de la arquitectura el 2026-08-31, reemplazado por Google/Gemini. **Neon** (base de datos PostgreSQL gestionada) y **Vercel** (hosting/despliegue) se retiraron de la arquitectura el 2026-08-28 tras la migración de stack a Django/SQL Server — cualquier obligación pendiente con estos tres proveedores retirados queda acotada al período en que sí procesaron datos; su baja efectiva como cuentas/proyectos activos es una gestión operativa del responsable del tratamiento, fuera del alcance técnico del sistema.
