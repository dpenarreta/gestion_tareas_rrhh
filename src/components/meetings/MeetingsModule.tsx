        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden max-h-40 overflow-y-auto">
              {users.length === 0 ? (
                <p className="text-sm text-slate-400 text-center py-4">Cargando usuarios…</p>
              ) : (
                users.map((u) => (
  ...
  return (
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[92vh] overflow-y-auto">
        {/* Header */}
        <div className="sticky top-0 bg-white px-6 py-4 border-b border-slate-200 flex items-start justify-between gap-4 rounded-t-2xl">
          <div className="min-w-0">
            ...
            <h2 className="text-lg font-bold text-slate-900 leading-tight">{meeting.title}</h2>
          </div>
        ...
        <div className="px-6 py-5 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <InfoRow label="Fecha" value={fmtDate(meeting.meetingDate)} />
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {meeting.invitees.map((inv) => (
                  <div key={inv.id} className="flex items-center gap-2 px-3 py-1.5 bg-slate-50 rounded-xl border border-slate-100">
                    <Avatar name={inv.user.name} size="sm" />
                    <span className="text-sm text-slate-700">{inv.user.name}</span>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="flex gap-2">
                {STATUS_OPTIONS.map((s) => (
                  <button key={s} onClick={() => patch({ status: s })} disabled={saving}
                    className={`px-3 py-1.5 text-xs font-medium rounded-xl border transition-colors disabled:opacity-50 ${meeting.status === s ? "bg-indigo-600 text-white border-indigo-600" : "text-slate-600 border-slate-200 hover:border-slate-300"}`}>
                    {STATUS_CFG[s].label}
                  </button>
                ))}
              </div>
          ...
          <div className="bg-violet-50 border border-violet-200 rounded-2xl p-4">
            ...
            <div className="flex items-center gap-3">
              <button onClick={copyOtterEmail}
                className="flex items-center gap-1.5 text-xs font-medium text-violet-700 bg-white border border-violet-200 px-3 py-1.5 rounded-lg hover:bg-violet-50 transition-colors">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Resumen y notas</p>
              {isHost && !editSummary && (
            ...
            {editSummary ? (
              <div className="space-y-3">
                <textarea rows={6} value={summaryText} onChange={(e) => setSummaryText(e.target.value)}
                  ...
                  className="w-full px-3 py-2.5 text-sm text-slate-900 bg-white border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-300 resize-y" />
              ...
              <div className="space-y-3">
                <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                  <p className="text-sm text-slate-700 whitespace-pre-wrap leading-relaxed">{meeting.otterSummary}</p>
                </div>
...
function MeetingCard({ meeting, currentUserId, onClick }: { meeting: Meeting; currentUserId: string; onClick: () => void }) {
  const isHost = meeting.hostId === currentUserId;
  return (
    <button onClick={onClick} className="w-full text-left bg-white border border-slate-200 rounded-2xl p-5 hover:border-indigo-200 hover:shadow-sm transition-all group">
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {isHost && (
            <span className="text-xs px-2 py-0.5 rounded-full border border-slate-200 bg-slate-50 text-slate-500 font-medium">
              Anfitrión
            </span>
          )}
        </div>
        <svg className="w-4 h-4 text-slate-300 group-hover:text-indigo-400 transition-colors shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
        </svg>
      </div>

      <h3 className="text-base font-semibold text-slate-900 mb-2 leading-snug">{meeting.title}</h3>
      ...
      {meeting.invitees.length > 0 && (
        <div className="flex items-center gap-1">
          ...
          {meeting.invitees.length > 5 && (
            <span className="text-xs text-slate-400 ml-1">+{meeting.invitees.length - 5}</span>
          )}
        </div>
      )}
    </button>
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">Zoom + notas automáticas con Otter.ai</p>
        </div>
        {canCreate && (
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-slate-500 font-medium">No tienes reuniones programadas</p>
          {canCreate && (
            <button onClick={() => setShowForm(true)} className="mt-4 text-sm text-indigo-600 hover:text-indigo-800 font-medium">
      ...
      {showForm && <MeetingFormModal onClose={() => setShowForm(false)} onSaved={handleSaved} />}
      {selected && (
        <MeetingDetailModal
          meeting={selected}
