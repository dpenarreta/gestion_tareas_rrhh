...
export default function AddViewPanel({ availableViews, onAdd, onClose }: Props) {
  ...
  return (
    <div
      ref={ref}
      className="absolute top-full left-0 mt-1 z-50 bg-surface border border-border rounded-2xl shadow-xl p-4 w-72"
    >
      <p className="text-[11px] font-semibold text-disabled uppercase tracking-wider mb-3">
        Agregar vista
      </p>
        ...
        {availableViews.map((view) => {
          ...
          return (
            <button
              key={view}
              onClick={() => onAdd(view)}
              className="flex items-center gap-3 p-3 rounded-xl border border-border hover:border-primary/40 hover:bg-primary-surface transition-all text-left group"
            >
              <div className="shrink-0 p-1.5 rounded-lg bg-background group-hover:bg-surface transition-colors">
                {info.icon}
              </div>
              <div>
                <p className="text-sm font-semibold text-title">{info.label}</p>
