...
// `EquilibrioView` (backend, Fase 16) — mismo patrón que
...
export async function GET(request: Request, ctx: Ctx) {
  ...
  const response = await djangoApiFetch(`/analytics/equilibrio/${userId}/`);
  if (!response) {
  ...
  if (!response.ok) {
    return NextResponse.json({ error: "Error al obtener Equilibrio Operativo" }, { status: response.status });
  }
