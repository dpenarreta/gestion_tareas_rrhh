...
export async function POST(request: NextRequest) {
  ...
  const response = await djangoApiFetch("/tasks/target-time/bulk-validate/", {
    method: "POST",
    body: JSON.stringify({
  ...
  const data = (await response.json()) as { updated_count: number; skipped_self_assigned: number[] };
